...
def draw_hud(frame, arah, koreksi_m, n_red, n_green, n_obstacle, fps):
    ...
    overlay = frame.copy()
    cv2.rectangle(overlay, (0, 0), (450, 160), COLOR_BLACK, -1)
    cv2.addWeighted(overlay, 0.55, frame, 0.45, 0, frame)

    # Arah
    arah_color = COLOR_CYAN if arah == "LURUS" else COLOR_YELLOW
    cv2.putText(
        frame, f"ARAH : {arah}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, arah_color, 2
    )

    cv2.putText(
        frame,
        f"Obstacle    : {n_obstacle}",
        (10, 106),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        COLOR_ORANGE if n_obstacle > 0 else COLOR_WHITE,  # ORANGE jika terdeteksi
        2,
    )
    ...
    if arah != "LURUS":
        ...
        cv2.putText(
            frame, ket, (10, 155), cv2.FONT_HERSHEY_SIMPLEX, 0.55, COLOR_YELLOW, 1
        )
...
def main():
    ...
    while True:
        if not paused:
            ...
            # no buoy detected
            if not buoy_red and not buoy_green:
                cv2.putText(
                    frame,
                    "WARNING: No buoy detected!",
                    (frame_w // 2 - 100, 50),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.9,
                    COLOR_RED,
            ...
            # Garis deadzone
            zone_left = center_x - DEAD_ZONE_PX
            zone_right = center_x + DEAD_ZONE_PX
            cv2.rectangle(
                frame, (zone_left, 0), (zone_right, frame_h), (160, 220, 255), 1
            )
            ...
            if midpoint_x is not None:
                ...
                cv2.putText(
                    frame,
                    f"Offset: {offset_px:+d}px",
                    (frame_w - 175, frame_h - 5),  # pojok kanan bawah
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.45,
                    offset_color,
                    1,
        ...
        # ---- Tampilkan ----
        cv2.imshow(
            "ASV Navigation System - RoboCamp 2026 (q=quit, p=pause, +/-=speed)", frame
        )
